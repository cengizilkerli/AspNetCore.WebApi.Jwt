    ...
    public class ProductApiManager : IProductApiService
    {
        ...
        public async Task AddAsync(ProductAdd productAdd)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.PostAsync("http://localhost:53308/api/products", stringContent);
            }
        }
        ...
        public async Task<List<ProductList>> GetAllAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.GetAsync("http://localhost:53308/api/products");
        ...
        public async Task<ProductList> GetByIdAsync(int id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.GetAsync($"http://localhost:53308/api/products/{id}");
